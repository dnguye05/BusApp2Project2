      ...
      *AUTHOR  : Gail Fulenwider, modified by David Nguyen             *
      *DATE    : 01/24/2022                                            *
      *ABSTRACT: Ctrl break on dept, unordered input data file.        *
      ******************************************************************
       IDENTIFICATION DIVISION.
       PROGRAM-ID. NGUYEN-P02-SORT.
      *-----------------------------------------------------------------
       ...
       FILE-CONTROL.
           ...
           SELECT SORT-WORK ASSIGN TO 'p02-data-sortwork.dat'.

           SELECT PR-RPT  ASSIGN TO 'NGUYEN-p02-sort.rpt'
                          ORGANIZATION IS LINE SEQUENTIAL.
      *-----------------------------------------------------------------
       DATA DIVISION.
       FILE SECTION.
       ...
       SD  SORT-WORK.
       01  SORT-REC.
           03  SORT-EMP-DEPT               PIC 99.
           03  SORT-EMP-ID                 PIC X(4).
           03  SORT-EMP-NAME.
               05  SORT-EMP-LNAME          PIC X(15).
               05  SORT-EMP-FNAME          PIC X(15).
           03  SORT-EMP-QTR-PAY            PIC 9(5)V99.
      ...
      *-----------------------------------------------------------------
       ...
       01  WS-RPT-TITLE-LN.
           03  FILLER      PIC X(28) VALUE 'P02-NGUYEN'.
      ...
      *-----------------------------------------------------------------
       ...
       100-MAIN.
           ...
           SORT SORT-WORK ON ASCENDING KEY    SORT-EMP-DEPT
                             ASCENDING KEY    SORT-EMP-LNAME
                             USING            EMP-MST
      ...
      *-----------------------------------------------------------------
       200-PRT-RPT.    
           ...
           PERFORM UNTIL EOF
               RETURN SORT-WORK
                   ...
                   NOT AT END
                       ADD 1 TO WS-REC-CTR
                       IF FIRST-REC
                           MOVE SORT-EMP-DEPT TO WS-SV-DEPT
                           MOVE 'N'           TO WS-FIRST-FLAG
                       END-IF
                       IF SORT-EMP-DEPT NOT EQUAL TO WS-SV-DEPT
           ...
           DISPLAY 'PROJECT 2 SORT - DAVID NGUYEN'.
      ...
      *-----------------------------------------------------------------
       500-WRITE-DTL.
           MOVE  SORT-EMP-DEPT                    TO   WS-DTL-DEPT-NUM.
           MOVE  WS-DEPT-NAME (SORT-EMP-DEPT)     TO   WS-DTL-DEPT-NAME.
           MOVE  SORT-EMP-ID                      TO   WS-DTL-ID.
           MOVE  SORT-EMP-LNAME                   TO   WS-DTL-LNAME.
           MOVE  SORT-EMP-FNAME                   TO   WS-DTL-FNAME.
           MOVE  SORT-EMP-QTR-PAY                 TO   WS-DTL-PAY              
           WRITE PR-RPT-REC                       FROM WS-DTL-LN.
           ADD   SORT-EMP-QTR-PAY                 TO   WS-DEPT-TOT.
      *-----------------------------------------------------------------
       600-CHG-DEPTS.
